...
module.exports.execute = async (client, message, args) => {
    ...
    if (secim === 'sıfırla') {
        if (!args[1] || (args[1] !== 'ses' && args[1] !== 'chat' )) return message.channel.send(embed.setDescription('Sıfırlanacak veriyi belirtmelisin! (ses/chat)')).then(x => x.delete({timeout: 5000}));
        if (args[1] === 'ses') {
            let newData = new Map();
    ...
    if (!secim) return message.channel.send(embed.setDescription('Ses, chat veya teyit istatistiklerini sıfırlamak istiyorsan **sıfırla ses/chat** bir ayar yapmak istiyorsan aşağıdaki seçimleri kullanmalısın.'));
};

module.exports.configuration = {
    name: 'sıfırla',
    aliases: ['statsıfırla','verisıfırla'],
