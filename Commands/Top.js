...
module.exports.execute = async(client, message, args,ayar) => {
   ...
   // if(!message.member.roles.cache.array().some(rol => message.guild.roles.cache.get(ayar.staffrole).rawPosition <= rol.rawPosition)) return  message.reply("`Bu komut yetkililere özeldir.`");
    ...
    MemberStats.find({guildID: message.guild.id}).exec((err, data) => {
        ...
        embed.setDescription(`${message.guild.name} | Genel ve Haftalık chat-ses istatistikleri;`);
        embed.addField('Haftalık Top Ses ', haftalikSesSiralama);
        embed.addField('Haftalık Top Chat ', haftalikChatSiralama);
        embed.addField('Genel Top Ses ', toplamSesSiralama);
        embed.addField('Genel Top Chat ', toplamChatSiralama);
